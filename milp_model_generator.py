...
class MILPModel:
    ...
    def _add_constraints_describing_connections(self, layer: Dense):
        ...
        layer_units = self._find_layer_units(i)

        is_last_layer = (i == len(self._network.layers) - 1)
        if is_last_layer:
            output_size = self._network.output_shape[1]
            for j in range(output_size):
                self._add_constraint_describing_unit(layer_units[j])
        else:
            layer_size = layer.units
    ...
    def _add_constraint_describing_unit(self, unit: Var):
        ...
        slack_variable = self._find_layer_slack_variables(layer_index)[unit_index]
